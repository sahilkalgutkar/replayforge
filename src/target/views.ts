...
// Markup is intentionally old-fashioned: nested layout tables, <font> tags, no
// ARIA, no test ids, and most labels in the cell next to the input rather than
// in a <label>. Several inputs end up with no accessible name at all.
...
function banner(message: string, background: string, color = '#000000'): string {
  return `<table width="100%" cellpadding="4" cellspacing="0" border="0" bgcolor="${background}">
<tr><td><font face="Verdana,Arial" size="2" color="${color}"><b>${esc(message)}</b></font></td></tr>
</table><br>`;
}
...
export function loginPage(profile: TenantProfile, session: Session, error?: string): string {
  return page(
    profile,
    'Sign On',
    `${error ? banner(error, '#ffe8e8', '#990000') : ''}
...
export function searchForm(profile: TenantProfile, session: Session, message?: string): string {
  // No id, no <label>, no title on the input. The only text near it is the
  // cell to its left.
  return page(
    profile,
    `${profile.memberWord} Search`,
    `<b>${esc(profile.memberWord)} Search</b><br><br>
${message ? banner(message, '#fff6d8') : ''}
...
export function searchResults(
  ...
  if (members.length === 0) {
    return page(
      ...
      `${profile.memberWord} Search`,
      `<b>${esc(profile.memberWord)} Search</b><br><br>
${banner(`No records found for "${query}".`, '#fff6d8')}
<a href="/content/member-search">Back to search</a>`,
    );
  }
  const rows = members
    .map(
...
<tr><td><font face="Verdana,Arial" size="2"><b>Sub-account opened.</b></font></td></tr></table>
<br>
<table cellpadding="3" cellspacing="0" border="0">
...
<br><a href="/content/member/${esc(memberNumber)}/accounts">Return to accounts</a>`,
  );
}
