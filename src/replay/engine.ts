...
import type { Assertion, CapabilityArtifact, Guard, OutcomeSpec, Risk, Step } from '../artifact/schema.js';
...
// Replays a recorded capability with no model involved. Every branch a run can
// take was declared in the artifact, which is what makes it repeatable and what
// makes a failure explainable by pointing at a line in the file.
//
// Three kinds of surprise are handled separately:
// - declared outcomes, checked after every screen, which end the run with an
//   answer ("no such member");
// - guards and retries, both capped, for things a run may shrug off;
// - everything else, which stops the run and reports the step, what was
//   expected, what was on screen, and a capture.
// Anything needing a person goes out through the escalation port.
...
export interface ReplayOptions {
  ...
  /** Values from the tenant's setup, e.g. `baseUrl`. */
  readonly variables?: Readonly<Record<string, string>>;
  readonly runId?: string;
  readonly escalation?: EscalationPort;
  readonly screenshots?: ScreenshotPolicy;
  /** A person has OK'd this call's risky steps up front. */
...
type StepOutcome =
  | { kind: 'continue'; observation: Observation }
  | { kind: 'terminal'; result: ReplayResult };

type MatchedOutcome = { name: string; description: string; disposition: 'answer' | 'needs_human' };
...
class ReplayRun {
  ...
  private readonly reports: StepReport[] = [];
  ...
  private bindings: RunBindings = { variables: {}, secrets: {} };
  private params: Record<string, ExtractedValue> = {};
  private steps: readonly Step[] = [];
  private outcomes: readonly OutcomeSpec[] = [];
  private successCheckpoint!: Assertion;
  private preconditions: readonly Assertion[] = [];
  private lastInterventionId = '';
  /**
   * Someone took the session, looked at what the run was about to do and handed
   * it back. That's approval for this call; otherwise the next attempt would
   * escalate the same step to the same person.
   */
  ...
  async execute(): Promise<ReplayResult> {
    const rejected = await this.prepare();
    if (rejected) return rejected;
    try {
      return await this.runSteps();
    } catch (error) {
      return this.fail({ category: 'surface_error', message: error instanceof Error ? error.message : String(error) });
    }
  }

  // --- setup: everything that can be checked without touching the app -----
  ...
  private async prepare(): Promise<ReplayResult | undefined> {
    ...
    this.redactor = new Redactor({ secrets: secrets.ok ? secrets.secrets : {}, piiValues });
    ...
    if (!validated.ok) {
      ...
      return this.fail({
        ...
        message: validated.issues.map((issue) => `${issue.name} ${issue.message}`).join('; '),
      });
    }
    if (!secrets.ok) {
      ...
      return this.fail({
        ...
        message: `missing credentials: ${secrets.missing.join(', ')}`,
      });
    }
    ...
    // Targets and checks may mention the caller's arguments, so fill those in
    // once here and nothing further down has to think about templates.
    this.steps = resolved.artifact.steps.map((step) => this.materialise(step));
    this.outcomes = this.artifact.outcomes.map((outcome) => ({
      ...outcome,
      when: deepSubstitute(outcome.when, this.params, this.bindings),
    }));
    this.successCheckpoint = deepSubstitute(this.artifact.successCheckpoint, this.params, this.bindings);
  ...
  /** Fills arguments into a step's targets and checks, leaving value sources alone. */
  private materialise(step: Step): Step {
    const { action } = step;
    let materialised: Step['action'] = action;
    if ('target' in action && action.target) {
      materialised = { ...action, target: deepSubstitute(action.target, this.params, this.bindings) } as Step['action'];
    }
    if (action.kind === 'waitFor') {
      materialised = { ...action, assertion: deepSubstitute(action.assertion, this.params, this.bindings) };
    }
    return {
      ...step,
      action: materialised,
  ...
  private async runSteps(): Promise<ReplayResult> {
    if (this.steps.length > this.artifact.policy.maxSteps) {
      return this.fail({
        category: 'budget_exceeded',
        message: `the flow has ${this.steps.length} steps but its policy allows ${this.artifact.policy.maxSteps}`,
      });
    }
    ...
    for (const precondition of this.preconditions) {
      const check = evaluateAssertion(precondition, observation);
      if (!check.ok) {
        return this.fail({
          category: 'checkpoint_failed',
          message: `precondition not met: ${check.detail}`,
    ...
    for (const step of this.steps) {
      if (this.now() - this.startedAt > this.artifact.policy.maxDurationMs) {
        return this.fail({
          category: 'budget_exceeded',
          message: `the run went over its ${this.artifact.policy.maxDurationMs}ms budget`,
          stepId: step.id,
        });
      }
  ...
  private async runStep(step: Step, incoming: Observation): Promise<StepOutcome> {
    const startedAt = this.now();
    const maxAttempts = step.retries.max + 1;
    const guardsFired: string[] = [];
    let observation = incoming;
    let escalationsUsed = 0;
    // Tracked apart from `attempt`, because a resume rewinds the attempt count
    // and the run must look at the screen again after a person has touched it.
    let firstPass = true;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      const before = this.matchOutcome(observation);
      if (before) {
        const handled = await this.handleOutcome(before, step, observation, escalationsUsed === 0);
      ...
      observation = await this.runGuards(step, observation, guardsFired);
      ...
      // Drift is measured against the screen this step acts on, not the one it
      // produces, or every step that changes anything would report drift.
      ...
      if (targetSpec) {
        ...
        if (!resolution.ok) {
          const detail = describeFailure(targetSpec, resolution);
          if (attempt < maxAttempts) {
            await this.recorder?.event('step.retrying', { stepId: step.id, attempt, reason: detail });
      ...
      const targetUrl =
        step.action.kind === 'navigate' ? resolveValue(step.action.url, this.params, this.bindings) : undefined;
      ...
      if (decision.verdict === 'block') {
        ...
        return {
          ...
          result: await this.failAtStep(step, observation, {
            ...
            expected: `an action allowed by ${decision.rule}`,
          }),
        };
      }
      ...
      const produced = this.matchOutcome(after);
      if (produced) {
        const handled = await this.handleOutcome(produced, step, after, escalationsUsed === 0);
        if (handled.kind === 'terminal') return handled;
        escalationsUsed += 1;
        attempt -= 1;
        continue;
      ...
      const location = this.policy.checkObservedLocation(after.url);
      if (location.verdict === 'block') {
        return {
          kind: 'terminal',
          result: await this.failAtStep(step, after, {
            category: 'policy_blocked',
            message: `after this step the session was somewhere it isn't allowed: ${location.reason}`,
          }),
        };
      }
      ...
      if (step.checkpoint) {
        ...
        if (!check.ok) {
          ...
          const handoff = await this.maybeEscalate(step, after, {
      ...
      if (drift) await this.recorder?.event('drift.detected', { stepId: step.id, ...drift });
    ...
    // Only reachable if the last attempt ended in a `continue`.
    return {
      kind: 'terminal',
      result: await this.failAtStep(step, observation, {
        category: 'checkpoint_failed',
        message: `step "${step.id}" ran out of attempts`,
      }),
    };
  }
    ...
    switch (action.kind) {
      ...
      case 'select':
        await this.surface.perform({
          ...
          value: resolveValue(action.value, this.params, this.bindings),
        });
        return;
      case 'setChecked':
        await this.surface.perform({ kind: 'check', ref: requireNode(node, step).ref, checked: action.checked });
        return;
      case 'pressKey':
        await this.surface.perform({ kind: 'press', key: action.key, ...(node ? { ref: node.ref } : {}) });
        return;
      case 'waitFor':
      ...
      case 'read': {
        ...
        if (raw === undefined || raw === '') {
          throw new Error(`step "${step.id}" read the ${action.from} of ${action.target.description} and found nothing`);
        }
  ...
  private async runGuards(step: Step, observation: Observation, fired: string[]): Promise<Observation> {
  ...
  private async applyGuard(guard: Guard, observation: Observation): Promise<Observation> {
    ...
    if (!resolution.ok) {
      // Not a failure: whatever the guard was for may already have cleared.
  ...
  private matchOutcome(observation: Observation): MatchedOutcome | undefined {
    for (const outcome of this.outcomes) {
      if (evaluateAssertion(outcome.when, observation).ok) {
        return { name: outcome.name, description: outcome.description, disposition: outcome.disposition };
      }
    }
    return undefined;
  }
  ...
  /**
   * A declared outcome fired. Returns `continue` only when a person took the
   * session, cleared the condition and handed it back; reporting the session as
   * expired after someone signed it back in would be wrong.
   */
  private async handleOutcome(
    ...
    this.reports.push(this.reportFor(step, 'skipped'));
  ...
  private async finish(observation: Observation): Promise<ReplayResult> {
    ...
    if (!success.ok) {
      return this.fail({
        category: 'checkpoint_failed',
        message: `every step ran but the success condition isn't met: ${success.detail}`,
        observed: this.redactor.redact(observation.text.slice(0, 400)),
        url: observation.url,
    ...
    for (const spec of this.artifact.outputs) {
      ...
      if (!coerced.ok) return this.fail({ category: 'output_invalid', message: coerced.message });
  ...
  private async maybeEscalate(
    ...
    await this.recorder?.event('escalation.resolved', {
      ...
      note: outcome.note,
      ...(outcome.resolution === 'resume'
        ? { operator: outcome.operator, humanActions: outcome.humanActions, actions: outcome.actions }
        : {}),
    });
    return outcome;
  }
  ...
  private async failAtStep(
    step: Step,
    observation: Observation,
    error: Pick<ReplayError, 'category' | 'message'> & Partial<ReplayError>,
  ): Promise<ReplayResult> {
    const screenshot = await this.recorder?.screenshot(`failure-${step.id}`, () => this.surface.screenshot(), 'failure');
    this.reports.push(this.reportFor(step, 'failed'));
    return this.fail({
      ...error,
      stepId: step.id,
      stepIntent: step.intent,
  ...
  private reportFor(step: Step, status: StepReport['status']): StepReport {
    return { stepId: step.id, intent: step.intent, status, risk: step.risk, attempts: 1, guardsFired: [], durationMs: 0 };
  }
...
function requireNode(node: UiNode | undefined, step: Step): UiNode {
  if (!node) throw new Error(`step "${step.id}" needs a control but none was resolved`);
  return node;
}
